...
export default function ExportModal({ onClose }: ExportModalProps) {
  ...
  const { data: categorias = [] } = useQuery<Categoria[]>({ queryKey: CATEGORIAS_KEY, queryFn: fetchCategorias })
  const hasError = lancError || fiadError || parcError

  async function handleExport(format: 'csv' | 'pdf') {
    setLoading(format)
    try {
      if (format === 'csv') exportCSV(lancamentos, fiados, parceiros, categorias)
      else exportPDF(lancamentos, fiados, parceiros, categorias)
      onClose()
    } finally {
      setLoading(null)
    }
